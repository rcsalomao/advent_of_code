...
def calc_total_happiness(persons: list[Person]):
    for i, person in enumerate(persons):
        if i == 0:
            person.set_left_person(persons[-1])
            person.set_right_person(persons[1])
        elif i == len(persons) - 1:
            person.set_left_person(persons[-2])
            person.set_right_person(persons[0])
        else:
            person.set_left_person(persons[i - 1])
            person.set_right_person(persons[i + 1])
...
def parte_1():
    ...
    for person_arrangement in persons_arrangements:
        happiness = calc_total_happiness(person_arrangement)
