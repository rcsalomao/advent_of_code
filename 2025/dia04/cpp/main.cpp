...
auto parte_1(vector<string>& lines) {
    ...
    long total{0};
    for (long i : vw::iota(0, max_i + 1)) {
        for (long j : vw::iota(0, max_j + 1)) {
            if (grid[i, j] != '@') continue;
            int sub_total{0};
            for (auto [di, dj] : neighbours) {
                long x{i + di};
                long y{j + dj};
                if (x < 0 or y < 0 or x > max_i or y > max_j) continue;
                if (grid[x, y] == '@') sub_total++;
            }
            if (sub_total < 4) total++;
        }
    }
        ...
        for (long i : vw::iota(0, max_i + 1)) {
            for (long j : vw::iota(0, max_j + 1)) {
                ...
                for (auto [di, dj] : neighbours) {
                    ...
                    long y{j + dj};
                    if (x < 0 or y < 0 or x > max_i or y > max_j) continue;
                    if (grid[x, y] == '@' or grid[x, y] == 'x') sub_total++;
                }
                if (sub_total < 4) {
                    grid[i, j] = 'x';
                    total++;
