...
void parte_1(std::vector<std::string>& lines) {
    ...
    int j{0};
    while (x0 + j * dx / 4 <= cell_n) {
        j++;
    }
    j--;
    int w{x0 + j * dx / 4};        // anchor
    int v{x0 + (j + 1) * dx / 4};  // next anchor
    int u{cell_n - w};
    int t{v - cell_n};
    int s = std::max(t, u);
...
void parte_2(std::vector<std::string>& lines) {
    string res =
        "Consultar o valor numérico imediatamente superior ao valor do input "
        "no site: https://oeis.org/A141481/b141481.txt";
